...
<!--Top nav var insert finish -->
		...
		<main> 
        	<div class="jumbotron text-center" style="margin:50px auto 30px ">
  				<h1>We have Wine & Rose </h1>
  				<h5>Select your favorite Wine or Rose here and add to shopping cart!</h5> 
			</div>
		...
		<h2><center> Wine & Rose </center></h2>
	
	
		<?php   //no touch from here 
			$product_query= "SELECT category_description FROM category WHERE category_id=3";
			...
			$wine_query= "SELECT * FROM product WHERE category_id=3 AND unit_in_stock > 0 ORDER BY product_id ASC";
			$wine_run = mysqli_query($dbc, $wine_query);
			$wine_count = mysqli_num_rows($wine_run);   // to here 
			
			if($wine_count>0){
				/*echo "<div> Currently, there are $wine&rose_count Wine and Rose available for you. </div> <hr>"; */
				echo"<hr>";
				echo "<div class='container' style= float: left; width: 50%; padding: 0 5>
						<div class='row' id='record'>";
				
						while($row = mysqli_fetch_array($wine_run, MYSQLI_ASSOC)){  // no touch from here
							
							echo"
								<div class='col-6' ><br><br>
																	
									<div><img src=".$row['product_picture']." height=400 ></div><br>
		...
		<?php
				...
				echo "<p> Sorry, there's no wine or rose available currently. </p>";
			}
...
<hr>
  ...
  <p>
	We hope you enjoy your time at Chapel bar. Thank you for your visit. 
  </p>
