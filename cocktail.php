...
<!--Top nav var insert finish -->
		...
		<main> 
        	<div class="jumbotron text-center" style="margin:50px auto 30px;">
  				<h1>We have cocktails </h1>
  				<h5>Select your favorite cocktail here and add to shopping cart!</h5> 
			</div>

        </main>
      <table>

		<h2><center> Cocktail </center></h2>
	
		<?php   //no touch from here 
			$product_query= "SELECT category_description FROM category WHERE category_id=2";
			...
			$cocktail_query= "SELECT * FROM product WHERE category_id=2 AND unit_in_stock > 0 ORDER BY product_id ASC";
			$cocktail_run = mysqli_query($dbc, $cocktail_query);
			$cocktail_count = mysqli_num_rows($cocktail_run);   // to here 
			
			if($cocktail_count>0){
				/*echo "<div> Currently, there are $cocktail_count cocktails available for you. </div> <hr>"; */
				echo"<hr>";

				echo "<div class='container'>
						<div class='row' id='record'>";
				
						while($row = mysqli_fetch_array($cocktail_run, MYSQLI_ASSOC)){  // no touch from here
		...
		<?php
				...
				echo "<p> Sorry, there's no cocktail available currently. </p>";
			}
