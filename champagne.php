...
<!--Top nav var insert finish -->
		...
		<main> 
        	<div class="jumbotron text-center" style="margin:50px auto 30px;">
  				<h1>We have Champagne </h1>
  				<h5>Select your favorite champagne here and add to shopping cart!</h5> 
			</div>

        </main>
      <table>

		<h2><center> Champagne </center></h2>
	
		<?php   //no touch from here 
			$product_query= "SELECT category_description FROM category WHERE category_id=4";
			...
			$champagne_query= "SELECT * FROM product WHERE category_id=4 AND unit_in_stock > 0 ORDER BY product_id ASC";
			$champagne_run = mysqli_query($dbc, $champagne_query);
			$champagne_count = mysqli_num_rows($champagne_run);   // to here 
			
			if($champagne_count>0){
				/*echo "<div> Currently, there are $champagne_count champagnes available for you. </div> <hr>"; */
				echo"<hr>";
				
				echo "<div class='container'>
						<div class='row' id='record'>";
				
						while($row = mysqli_fetch_array($champagne_run, MYSQLI_ASSOC)){  // no touch from here
		...
		<?php
				...
				echo "<p> Sorry, there's no champagne available currently. </p>";
			}
